...
class RemoveMoney extends PluginCommand{
    ...
    {
        parent::__construct("removemoney", $main);
        $this->setPermission("removemoney.use");
    }

    public function execute(CommandSender $player, string $commandLabel, array $args)
    {
        if (!$player->hasPermission("removemoney.use")){
        ...
        MoneyAPI::removeMoney($sender, $args[1]);

        if (($sender instanceof Player) and ($sender->getName() != $player->getName())) {
            LanguageAPI::sendMessage($sender, "§a{$player->getName()} §fvient de vous retirer §a$args[1]§f$ !", "§a{$player->getName()} §fjust retired §a$args[1]§f$ !");
        }

        LanguageAPI::sendMessage($player, "Vous venez de retirer§a $args[1]§f$ à §a$name §f!", "You just removed§a $args[1]§f$ to §a$name §f!");
    }
}