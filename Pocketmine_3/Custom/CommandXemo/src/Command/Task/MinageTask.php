...
class MinageTask extends Task{
    ...
    public function onRun(int $currentTick) {
        ...
        if (($Px != $x) or ($Py != $y) or ($Pz != $z)) {
            ...
            $player->sendTip($config->get("bouge.minage"));
        ...
        if ($this->time === 0) {
            ...
            $son->z = $player->z;
            $player->sendDataPacket($son);
            $player->sendMessage($config->get("minage.yes"));
            $x = $config->get("minage")[0];
            $y = $config->get("minage")[1];
            $z = $config->get("minage")[2];
            $monde = $config->get("minage")[3];
            ...
            $custommessage = $config->get("minage.tp");
