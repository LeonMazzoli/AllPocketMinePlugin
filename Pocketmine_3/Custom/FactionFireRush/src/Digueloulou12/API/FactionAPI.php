...
class FactionAPI
{
    ...
    {
        self::$config = new Config(Main::getInstance()->getDataFolder() . "faction.json", Config::JSON);
        self::$power = new Config(Main::getInstance()->getDataFolder() . "power.json", Config::JSON);
        self::$claim = new Config(Main::getInstance()->getDataFolder() . "claim.json", Config::JSON);

        Main::getInstance()->getLogger()->info("Faction on by Digueloulou12");
    ...
    public static function isInFaction($player): bool
    {
        ...
        foreach (self::$config->getAll() as $fac_name => $key) {
            foreach (self::$config->get("$fac_name")["Members"] as $member_name => $rank) {
                if ($member_name === Main::getPlayerName($player)) $return = true;
            }
        }
    ...
    {
        self::$config->set($fac_name, ["Members" => [Main::getPlayerName($player) => "Owner"], "desc" => " ", "power" => self::$power->get(Main::getPlayerName($player)), "home" => " ", "ally" => []]);
        self::$config->save();
    }
    ...
    public static function getFactionPlayer($player): string
    {
        ...
        foreach (self::$config->getAll() as $fac_name => $key) {
            foreach ($key["Members"] as $name => $rank) {
                if ($name === Main::getPlayerName($player)) $return = $fac_name;
            }
        }
    ...
    {
        self::addPowerFaction($fac_name, self::$power->get(Main::getPlayerName($player)));
        self::$config->setNested("$fac_name.Members." . Main::getPlayerName($player), "Membre");
        self::$config->save();
    }

    public static function getRankPlayerChatInFaction($player, string $fac_name): string
    {
        $rank = self::$config->get($fac_name)["Members"][Main::getPlayerName($player)];
    ...
    public static function sendMessageFaction(string $msg, string $fac_name, $player_)
    {
        foreach (self::$config->get($fac_name)["Members"] as $player => $rank) {
            ...
            if ($sender instanceof Player) {
                $sender->sendMessage(ConfigAPI::getConfigReplace("chat", ["{rank}", "{player}", "{faction}", "{msg}"], [self::getRankPlayerChatInFaction($sender, self::getFactionPlayer($sender)), Main::getPlayerName($player_), $fac_name, $msg]));
            }
        }
    }
    ...
    {
        self::removePowerFaction($fac_name, self::$config->get(Main::getPlayerName($player)));
        self::$config->removeNested("$fac_name.Members." . Main::getPlayerName($player));
        self::$config->save();
    }
    ...
    public static function getMaxPowerFaction(string $fac_name): int
    {
        return self::countPlayerInFaction($fac_name) * ConfigAPI::getConfigValue("max_power_player");
    }
    ...
    {
        $members = implode(", ", self::getMembersFaction($fac_name));
        return ConfigAPI::getConfigReplace("msg_info", ["{power}", "{name}", "{owner}", "{members}", "{desc}", "{max_power}", "{claim}"], [self::getPowerFaction($fac_name), $fac_name, self::getOwnerPlayer($fac_name), $members, self::$config->getNested("$fac_name.desc"), self::getMaxPowerFaction($fac_name), self::countClaim($fac_name)]);
    }
    ...
    public static function setOwnerFaction($player, string $fac_name)
    {
        ...
        self::$config->setNested("$fac_name.Members." . Main::getPlayerName($player), "Owner");
        self::$config->save();
    }

    public static function renameFaction(string $fac_name, string $new_name)
    {
        foreach (self::$claim->getAll() as $claim => $fac) {
            if ($fac_name === $fac) self::$claim->set($claim, $new_name);
            self::$claim->save();
        }

        self::$config->set($new_name, self::$config->get($fac_name));
        self::$config->remove($fac_name);
        self::$config->save();
    }
    ...
    {
        self::$config->setNested("$fac_name.Members." . Main::getPlayerName($player), "*");
        self::$config->save();
    }

    public static function demotePlayerInFaction($player, string $fac_name)
    {
        self::$config->setNested("$fac_name.Members." . Main::getPlayerName($player), "");
        self::$config->save();
    }
    ...
    public static function sendTopFaction(Player $player)
    {
        ...
        foreach ($power as $fac => $value) {
            if ($i !== ConfigAPI::getConfigValue("top_faction") + 1) {
                $fac_ = "$fac";
                if (FactionAPI::getFactionPlayer($player) === $fac) $fac_ = "§9$fac";

                $player->sendMessage(ConfigAPI::getConfigReplace("info_top", ["{top}", "{faction}", "{power}"], [$i, $fac_, $value]));
                $i++;
            } else break;
        }
    }
    ...
    public static function claimChunk(Chunk $chunk, string $fac_name, bool $owerclaim = false)
    {
        if ($owerclaim === true) self::removePowerFaction($fac_name, ConfigAPI::getConfigValue("overclaim_cost")); else self::removePowerFaction($fac_name, ConfigAPI::getConfigValue("power_per_claim"));
    ...
    public static function unclaimChunk(Chunk $chunk, bool $owerclaim = false)
    {
        $pos = "{$chunk->getX()}:{$chunk->getZ()}";
        if ($owerclaim === true) self::addPowerFaction(self::$claim->get($pos), ConfigAPI::getConfigValue("power_per_claim") / 2); else self::addPowerFaction(self::$claim->get($pos), ConfigAPI::getConfigValue("power_per_claim"));
