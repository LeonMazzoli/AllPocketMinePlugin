...
class ReportAdmin extends PluginCommand
{
    ...
    {
        $command = explode(":", Main::getConfigAPI()->getConfigValue("reportadmin"));
        parent::__construct($command[0], Main::getInstance());
        if (isset($command[1])) $this->setDescription($command[1]);
        if (isset($command[2])) $this->setPermission($command[2]);
        $this->setAliases(Main::getConfigAPI()->getConfigValue("reportadmin_aliases"));
    }

    public function execute(CommandSender $player, string $commandLabel, array $args)
    {
        $info = explode(":", Main::getConfigAPI()->getConfigValue("reportadmin"));
        if ($player instanceof Player) {
            if (isset($info[2])) {
                if ($player->hasPermission($info[2])) {
                    ReportForms::reportAdmin($player);
                } else $player->sendMessage(Main::getConfigAPI()->getConfigValue("noperm"));
            } else ReportForms::reportAdmin($player);
