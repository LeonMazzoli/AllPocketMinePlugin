...
class RemovePermission extends Command
{
    ...
    {
        parent::__construct(Utils::getConfigValue("removepermission")[0]);
        if (isset(Utils::getConfigValue("removepermission")[1])) $this->setDescription(Utils::getConfigValue("removepermission")[1]);
        $this->setAliases(Utils::getConfigValue("removepermission_aliases"));
                    ...
                    if (RankAPI::existPlayer($name)) {
                        if (isset($args[2])) {
                            if (in_array($args[2], RankAPI::getPermission($name, true))) {
                                RankAPI::removePermission($name, $args[2], true);
                                $sender->sendMessage(Utils::getConfigReplace("removepermission_player_msg", ["{player}", "{perm}"], [$name, $args[2]]));
                            } else $sender->sendMessage(Utils::getConfigReplace("no_has_perm_player", ["{player}", "{perm}"], [$name, $args[2]]));
                    ...
                    if (RankAPI::existRank($args[1])) {
                        if (isset($args[2])) {
                            if (in_array($args[2], RankAPI::getPermission($args[1], false))) {
                                RankAPI::removePermission($args[1], $args[2], false);
                                $sender->sendMessage(Utils::getConfigReplace("removepermission_rank_msg", ["{rank}", "{perm}"], [$args[1], $args[2]]));
                            } else $sender->sendMessage(Utils::getConfigReplace("no_has_perm_rank", ["{rank}", "{perm}"], [$args[2], $args[2]]));
                        } else $sender->sendMessage(Utils::getConfigReplace("no_args_perm"));
