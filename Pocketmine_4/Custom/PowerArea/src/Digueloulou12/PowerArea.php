...
use Digueloulou12\Task\PowerTask;
use pocketmine\plugin\PluginBase;
use pocketmine\player\Player;

class PowerArea extends PluginBase
{
    private static PowerArea $main;
    ...
    public function onEnable(): void
    {
        ...
        } else $this->getScheduler()->scheduleRepeatingTask(new PowerTask(), 20 * $this->getConfig()->get("time"));
    }
    ...
    public function addPower(Player $player, int $amount): void
    {
        $plugin = $this->getServer()->getPluginManager()->getPlugin("PiggyFactions");
        $damagerMember = $plugin->getPlayerManager()->getPlayer($player);
        if ($damagerMember !== null) $damagerMember->setPower($amount);
    }

    public static function getInstance(): PowerArea
    {
        return self::$main;
    }
}